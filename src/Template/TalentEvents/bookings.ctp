...
<div class="container content-container">
    <div class="row">
        <a class="goBackLink" onclick="return window.history.back();" style="float:right;"><i class="fa fa-angle-left" aria-hidden="true"></i>&nbsp;Go back</a>
        <div class="" id="no-more-tables">
            <table class="table-bordered table-condensed cf">
                ...
                <?php
                if (count($talentEvents)) {
                    ?>
                    <tbody>
                        <?php
                        foreach ($talentEvents as $key => $talentEvent):
                            $tr_class = '';
                            if ($talentEvent->payment) {
                            ...
                            <tr class="<?= $tr_class ?>">
                                ...
                                <td data-title="Action">
                                    <button  data-toggle="tooltip" title="View booking details" class="btn btn-info" onclick="show_detail(<?= $talentEvent->id; ?>)"><i class="fa fa-eye"></i></button>
...
//                                    echo $this->Html->link(__('<i class="fa fa-edit"></i>'), array('controller' => 'TalentEvents', 'action' => 'eventedit', $talentEvent->id, $this->request->params['pass'][0]), array('class' => 'btn btn-success', 'escape' => false));
                ...
                <?php }else { ?>
                    <tr>
                        <td colspan="4" style="font-size:18px;text-align: left !important;color:red">No Booking Found</td>
                    </tr>
                <?php } ?>
            </table>
            <div class="paginator">
    ...
    <div class="modal fade" id="summary_modal" role="dialog">
        <div class="modal-dialog" style="width:700px">
            <div class="modal-content" style="background-color: #F5F5F5 !important;">
                <div class="modal-header" style="display:block !important;color:black;padding: 10px 15px 0 0;border-bottom: none;">
...
<script>
    function show_detail(booking) {
        $.ajax({
            type: 'GET',
            url: '<?php echo $this->request->webroot ?>TalentEvents/viewBooking',
