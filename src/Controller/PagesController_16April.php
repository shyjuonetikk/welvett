...
//STORE LOGS
                ...
                $activity = 'User Profile Inserted';
                $updatedUserName = $this->request->data['first_name'] . ' ' . $this->request->data['last_name'];
...
// output error message if fails
    ...
    public function categories($text = null, $category_id = null, $id = null) {
        ...
        $role = 4;
        $subc = "";
        if ($this->request->is('post') || !empty($_GET['text'])) {
            $condition = 'roles.id="' . $role . '"';
            if (!empty($_GET['text'])) {
            ...
            if (!empty($this->request->data['min-price']) && !empty($this->request->data['max-price'])) {
                $minprice = $this->request->data['min-price'];
                $maxprice = $this->request->data['max-price'];
                $condition .= ' AND event_types.amount between ' . $minprice . ' AND ' . $maxprice;
            } else if (!empty($this->request->data['min-price'])) {
                $minprice = $this->request->data['min-price'];
                $condition .= ' AND event_types.amount <=' . $minprice;
            } else if (!empty($this->request->data['max-price'])) {
                $maxprice = $this->request->data['max-price'];
                $condition .= ' AND event_types.amount >=' . $maxprice;
            }

            $query = $conn->execute('SELECT tcal.date,users.id as user_id,talent_events.id as event_id,client_type,event_types.amount ,talent_event_subcategories.eventsubcategory_id,CONCAT(first_name, last_name),first_name,last_name,eventsubcategories.title as sub,image_icon,profile_image, loginwithsocial'
                    . ' FROM roles '
                    . 'join users  ON roles.id=users.role_id '
                    . ' join employee_members  ON users.id=employee_members.user_id '
                    ...
                    . 'where ' . $condition . ' AND (client_type="" OR client_type is NULL OR client_type="' . $this->Auth->user('role_id') . '" ) GROUP BY ' . $subc . '  users.id');
            ...
            $query = $conn->execute('SELECT users.id as user_id,talent_events.id as event_id,client_type,first_name,last_name,talent_event_subcategories.eventsubcategory_id,event_types.amount,eventsubcategories.title as sub,image_icon,profile_image, loginwithsocial'
                    . ' FROM roles '
                    . 'join users  ON roles.id=users.role_id '
                    . ' join employee_members  ON users.id=employee_members.user_id '
                    . ' join talent_event_subcategories  ON users.id=talent_event_subcategories.user_id '
                    ...
                    . ' '
                    . ' join event_types ON talent_events.id=event_types.talent_event_id '
                    . 'where roles.id="' . $role . '" AND (client_type="" OR client_type IS NULL OR client_type="' . $this->Auth->user('role_id') . '") and eventcategories.id="' . $category_id . '" GROUP BY talent_event_subcategories.eventsubcategory_id , users.id');
            $eventsubcategories = $this->Eventsubcategories->find('all', [
                'conditions' => ['Eventsubcategories.eventcategory_id' => $category_id]
            ]);
            $this->set(compact('eventsubcategories'));
        } else {
            $query = $conn->execute('SELECT users.id as user_id,talent_events.id as event_id,client_type,first_name,event_types.amount,last_name,talent_event_subcategories.eventsubcategory_id,eventsubcategories.title as sub,image_icon,profile_image, loginwithsocial'
                    . ' FROM roles '
                    . 'join users  ON roles.id=users.role_id '
                    . ' join employee_members  ON users.id=employee_members.user_id '
                    . ' join talent_event_subcategories  ON users.id=talent_event_subcategories.user_id '
                    ...
                    . ' '
                    . ' join event_types ON talent_events.id=event_types.talent_event_id '
                    . 'where roles.id="' . $role . '" AND (client_type="" OR client_type is NULL OR client_type="' . $this->Auth->user('role_id') . '")  GROUP BY talent_events.eventcategory_id, users.id');
        }
        $event_employees = $query->fetchAll('assoc');

        $this->set(compact('', 'eventcategories'));
        $this->set(compact('event_employees'));
